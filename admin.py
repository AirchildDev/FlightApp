...
# R = READ (Search button)
search_click = 0         # code 1
def search():
    global search_click  #code 2
    search_click += 1    #code 3
    flight_number = entry1.get()
    ...
    if flight_number:
        ...
        if result:
            ...
            message.showinfo("Prompt", "Record doesn't exist in the flights table")
            entry1.delete(0, tk.END)
            entry2.delete(0, tk.END)
            entry3.delete(0, tk.END)
            entry4.delete(0, tk.END)
...
# Clears what is displayed on screen
    if search_click == 2:        # code 4 (from line 79 to line 89)
        entry1.delete(0, tk.END)
        entry2.delete(0, tk.END)
        entry3.delete(0, tk.END)
        entry4.delete(0, tk.END)
        ...
        message.showinfo("Alert", "Cleared from screen")

    # Reset so it works again
        search_click = 0
            

# U = UPDATE (Update flight button) get what we need from the entries 
...
def update():
    ...
    if flight_number and origin and arrival_time and departure_time and destination:
        con = sqlite3.connect("Airline.db")
        cur = con.cursor()
        ...
        message.showinfo("Prompt", "Please enter your flight number")
...
def delete():
    ...
    if flight_number == "":
        message.showinfo("Prompt", "Please enter flight number to delete")
        ...
        if cur.rowcount > 0:
            message.showinfo("Alert", "Record deleted successfully")
...
btnDelete = tk.Button(leftframe, text= "Delete Flight", bg= "#ffffff", fg= "#000000", command= delete)
